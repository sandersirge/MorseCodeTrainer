...
from ..exceptions import SessionInvalidStateError, SessionNotInitializedError
...
class TestPresenter:
	...
	def record_answer(self, answer: str, elapsed: float) -> None:
		if self._session.current_prompt() is None:
			raise SessionInvalidStateError("Cannot record answer without an active prompt")
	...
	def _ensure_active_prompt(self) -> str:
		...
		if prompt is None:
			raise SessionNotInitializedError("No active prompt available")
		return prompt
